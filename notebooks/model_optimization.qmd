...
from sklearn.ensemble import ExtraTreesRegressor, AdaBoostRegressor, RandomForestRegressor, HistGradientBoostingRegressor
...
candidate_models = {
  "Ridge Regression": {
    ...
    "Extra Trees Regressor": {
        ...
        "pipeline": Pipeline(steps=[('preprocessor', preprocessor), ('dt', DecisionTreeRegressor())]),
        "param_grid": {
...
dataset_source = HTTPDatasetSource(url=dataset_source_url)
dataset_name = "ICRISAT DLD dataset"
dataset_target = "yield"
dataset_tags = {'description': "The District Level Database (DLD) for Indian agriculture and allied sectors provides a comprehensive one-stop shop for data on rural sector that enables testing of key research hypotheses, identification of relevant districts / regions for technology dissemination, promoting rural pro-poor programs / development initiatives and identification of relevant representative districts for micro level assessment." }
```


```{python}
dataset = mlflow.data.from_pandas(
    df=train_df, source=dataset_source, targets=dataset_target, name=dataset_name
)
```
...
```{python}
from sklearn.model_selection import cross_val_score
from optuna.samplers import TPESampler, RandomSampler, GPSampler, BruteForceSampler
model_results = {}
...
for model_name, model_params in search_spaces.items():
    with mlf.start_run(model_name, "HPO", f"{model_name}_Optimization") as parent_run:
        ...
        sampler = RandomSampler(seed=99999) #  Random Sampler 50 
        ...
        train_scores = cross_val_score(best_model, X_train, y_train, cv=tscv, scoring='neg_mean_absolute_error') 
...
def plot_train_test():
    ...
    val_scores = cross_val_score(best_model, X_val, y_val, cv=tscv, scoring='neg_mean_absolute_error')
...
## Training on Full Dataset before Deployment
```{python}
