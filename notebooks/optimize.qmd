...
from rice_yield.models.shap_utils import shap_explainer
...
We will use the train set for hyperparameter optimizaton, validation set for picking the optimal model and test set for deployment. There should be no overlap between the sets. Let\'s sort the data before splitting the data.
...
Due to the time-series nature of the dataset, we use time aware cross-validation which doesn't split the data randomly. So, the end year of the train set will always be a year less than the start of the test set. 
...
We use validation curves to find out the search space for each candidate model where the optimal value will be likely in.
...
def make_objective(model_name, model_class, param_fn):
    ...
    def objective(trial):
        ...
        with mlf.start_run(model_name=model_name, 
                           experiment_type="HPO", 
...
mlf.set_tracking_uri(f"file:///{output_path}/mlruns")
for model_name, model_params in search_spaces.items():
    with mlf.start_run(model_name, "HPO", f"{model_name}_optimization") as parent_run:
        create_folder(get_output_dir())
        study = opt.create_study(study_name=f"{model_name}_Study",
                                 direction="minimize",
                                 storage=f"sqlite:///{output_path}/models_db.sqlite3")
        ...
        save_model(best_model, model_name.lower(), output_folder)
...
* We can observe from the predicton error plot that, Random Forest model is better than the competing models but still makes large errors. 

* In the Residual vs Predicted plot of Random Forest model, we can observe that the model predicted more values in the range 2500 to 4000. And these points are further away from the zero line, differing about 500 - 1000 for most values. So, this level of performance is also expected when the model is deployed and tested using test set.

* We can also observe that the performance has plateaued for all the models and couldn\'t be improved further with the given feature set.

* It makes sense that prediction of rice yield also depends on other factors like soil quality, fertilize used, rice variety among other things. As these data are unavailabe on the ICRISAT site, we could assume that more data would help uncover real-world conditons in a better way and also improve model performance.
...
* In this plot, Random Forest model yield one of the lowest train and validation scores and the difference is smaller which indicates that there\'s no overfitting. 

* However, since the train and validation errors are high, we have to admit that the model has underfit the data. Getting more data should help overcome this issue.

* Finally, the best model (Random Forest) is chosen from competing models and before using it for deployment, we\'ll train on the full train + validation set.
...
# Chapter 9: SHAP Analysis

```{python}
full_train_df = pd.concat([X_train, X_val], axis=0)
render_table(full_train_df)
```

```{python}
#| output: false
best_model_path = get_output_dir()/"trained_models/randomforest.joblib"

shap_val = shap_explainer(best_model_path, full_train_df, "randomforest")
```

```{python}
summary_plot(shap_val, max_display=20)
```

```{python}
beeswarm_plot(shap_val, max_display=20)
```


<br>

We can infer the following from the beeswarm plot:

* The `area_ratio` feature shows mixed effect, though higher the value for this features, higher the shap values and hence contribute positively to the yield

* `water_deficit` is another feature that aligns with the agro knowledge. Low water deficit contributes greatly in a positive way to the yeid. Likewise, `act_etranspiration`.

* Low values of `avg_temperature` contribute positively to the yield while high values contribute
negatively.

* `rainfall` and `year` have mixed effect meaning that they don\'t contribute in one direction.

</br>

```{python}
data_dir = get_data_dir("final")
save_model(best_model, "randomforest", get_output_dir()/"best_model")
...
The best performing model has been saved and will be used for app deployment using Streamlit.