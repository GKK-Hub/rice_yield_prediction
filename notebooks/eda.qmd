...
---
The objective of this project is to predict rice yield using given parameters and the impact of the parameters on rice yield. This could help agronomists and agricultural scientists to make informed decisions.

So, we\'re using the ICRISAT District Level Database which is real-world database compiled with the data of various government agencies. Here\'s a short version about this data given on the official site.
...
Unfortunately, there\'s no API to download the data programatically. So, we\'ve downloaded the data manually, cleaned and combined into a single dataset.

In this file, we\'re going to explore the cleaned dataset by visualizing distribution of predictors and response, identifying correlation between variables, and checking data types. Finally, we\'ll do feature engineering where we engineer columns that might yield better results than the original ones.
...
from rice_yield.utils.dataframe_utils import rename_columns, write_csv
from rice_yield.utils.notebook_utils import render_table, render_print
from rice_yield.utils.paths import get_data_file, get_data_dir
from rice_yield.utils.plot_utils import show_correlation_heatmap, show_distribution
```
...
```{python}
df = rename_columns(df, rename_dictionary)
render_table(pd.DataFrame(df.columns, columns=['Variables/Attributes']))
```
...
<br>
We observed that `area` and `irrigated_area` has very strong correlation. Since rice is a water-intensive crop, this correlation makes sense. 

Similarly, `area` and `production` are infact strongly correlated. Also, `yield` is calculated by `production` / `area`. So,  multicollinearity creates problem when doing inference.
...
filtered_df['avg_temperature'] = round((filtered_df['max_temperature'] + filtered_df['min_temperature'] / 2), 2)
```
...
<br>
We\'re removing `area` and `irrigated_area` as we\'ve engineered a new column - `area_ratio, which gets a maxiumum value of 1 when `area` == `irrigated_area` and less than 1 when `irrigated_area` is less than the `area`.

`production` and `area` are highly corrleated and we\'re removing one of the columns to address multicollinearity.

We\'ve replaced `min_temperature` and `max_temperature` with the `avg_temperature`.

Also, `pot_etranspiration` and `precipitation` are removed due to the following definition from ICRISAT website:

<blockquote>
Climate Water deficit the cumulative difference between the potential evapotranspiration and precipitation during a specified period in which the precipitation is the smaller of the two
</blockquote>

So, we retain `water_deficit` and remove other two columns as we can infer the two variables\' effect from `water_defict`
</br>
